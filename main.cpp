...
glm::vec3 diffuseColor = lightColor * glm::vec3(0.5f);//0.5
glm::vec3 ambientColor = diffuseColor * glm::vec3(0.75f);//0.75
...
void LoadTextures()
{
	...
	t_smile = generateTextures("resources/Texturas/awesomeface.png", 1, true);
	t_piso = generateTextures("resources/Models/Piso/Mosaico.jpg", 0, true);
	//This must be the last
	t_white = generateTextures("resources/Texturas/white.jpg", 0, false);
...
void myData() {
	float vertices[] = {
		// positions          // texture coords
		 ...
		 0.5f, -0.5f, 0.0f,   0.5f, 0.6666f, // bottom right
		-0.5f, -0.5f, 0.0f,   0.25f, 0.6666f, // bottom left
	...
	float verticesPiso[] = {
		// positions          // texture coords
		 10.5f,  10.5f, 0.0f,   10.0f, 10.0f, // top right
		 10.5f, -10.5f, 0.0f,   10.0f, 0.0f, // bottom right
		-10.5f, -10.5f, 0.0f,   0.0f, 0.0f, // bottom left
		-10.5f,  10.5f, 0.0f,   0.0f, 10.0f  // top left 
	};
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Proyecto Final Computacion Grafica e Interaccion Humano-Computadora - Valencia", NULL, NULL);
	if (window == NULL) {
	...
	ModelAnim girlWalk("resources/objects/GirlWalk/Girl.dae");
	girlWalk.initShaders(animShader.ID);*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		staticShader.setVec3("dirLight.ambient", glm::vec3(0.5f));
		staticShader.setVec3("dirLight.diffuse", glm::vec3(0.7f));
		...
		glBindVertexArray(VAO[1]);
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(14.0f, 13.0f, -15.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 1.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_cubo);
		//glDrawArrays(GL_TRIANGLES, 0, 36); //A lonely cube :(
